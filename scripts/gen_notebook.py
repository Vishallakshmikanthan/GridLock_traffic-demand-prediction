"""
Generates notebooks/03_Ultimate_Model.ipynb with all source code populated.
Run with:  python scripts/gen_notebook.py
"""
import json, uuid, pathlib

OUT = pathlib.Path(__file__).parent.parent / "notebooks" / "03_Ultimate_Model.ipynb"
...
TITLE = """\
# Gridlock 2.0 — Ultimate Traffic Demand Prediction
## Complete 10-Section Advanced ML Pipeline

**Metric**: `score = max(0, 100 × R²)` &nbsp;|&nbsp; **Goal**: Maximise leaderboard R²
...
| 4 | Advanced Feature Engineering (100+ features) |
| 5 | Full Pipeline with INDEX-ALIGNMENT FIX |
| 6 | Optuna Hyperparameter Tuning (LGB / CB / XGB) |
| 7 | Out-of-Fold Predictions & Ensemble |
| 8 | SHAP + Feature Importance |
| 9 | Debug & Submission Validation |
| 10 | Final Submission + Leaderboard Tracker |

> **Critical fix built-in**: `add_lag_rolling_features()` re-sorts rows by time
> to compute lags, so `test_fe` row order no longer matches `test.csv`.
> This notebook restores the correct alignment before building `X_test`.\
"""
...
try:
    ...
    SHAP_AVAIL = False
    print("shap not available — SHAP section will be skipped")
...
plt.suptitle("Gridlock 2.0 — EDA Overview", fontsize=15, y=1.01)
plt.tight_layout()
...
def add_triple_stats(df_train, df_test):
    """Triple interaction: geohash x day x hour — most granular temporal-spatial feature."""
    df_train = df_train.copy(); df_test = df_test.copy()
...
axes[0].set_title("LightGBM — Top 30 Feature Importances", fontsize=13)
...
axes[1].set_title("CatBoost — Top 30 Feature Importances", fontsize=13)
...
if SHAP_AVAIL:
    ...
    plt.title("SHAP Summary — LightGBM (Top 25)", fontsize=13)
    plt.tight_layout()
...
print("=" * 55)
print("  DEBUG STEP 1 — Feature Matrix Diagnostics")
print("=" * 55)
...
DEBUG2 = '''\
print("=" * 55)
print("  DEBUG STEP 2 — Index Alignment Audit")
print("=" * 55)
...
print("=" * 55)
print("  DEBUG STEP 3 — Prediction Diagnostics")
print("=" * 55)
...
print("\\n" + "="*65)
print("  LEADERBOARD TRACKER — fill LB_Score after submission")
print("="*65)
...
cells = [
    ...
    code(LOAD_DATA),
    md("## Section 3 — Exploratory Data Analysis"),
    code(EDA),
    md("## Section 4 — Advanced Feature Engineering\n### 4A. Geospatial Utilities"),
    code(GEO_UTILS),
    md("### 4B. Timestamp Features"),
    code(TIME_FEATS),
    md("### 4C. Geospatial Features"),
    code(GEO_FEATS),
    md("### 4D. Weather & Road Features"),
    code(WEATHER_ROAD),
    md("### 4E. Statistical Aggregations (triple interaction, prefix stats)"),
    code(STAT_FEATS),
    md("### 4F. Lag & Rolling Features (INDEX-SAFE)"),
    code(LAG_FEATS),
    md("### 4G. Encodings & Interaction Features"),
    code(ENCODINGS),
    md("## Section 5 — Full Pipeline (INDEX-ALIGNMENT FIX built-in)"),
    code(PIPELINE),
    code(RUN_PIPELINE),
    md("### 5A. Build Feature Matrix"),
    code(FEATURE_MATRIX),
    md("## Section 6 — Validation Strategy"),
    code(VALIDATION),
    md("## Section 7A — Hyperparameter Tuning: LightGBM"),
    code(LGB_OPTUNA),
    md("## Section 7B — Hyperparameter Tuning: CatBoost"),
    code(CB_OPTUNA),
    md("## Section 7C — Hyperparameter Tuning: XGBoost"),
    code(XGB_OPTUNA),
    md("## Section 7D — Out-of-Fold Predictions"),
    code(LGB_OOF),
    code(CB_OOF),
    code(XGB_OOF),
    md("## Section 7E — Advanced Ensemble Methods"),
    code(ENSEMBLE),
    code(OOF_LEADERBOARD),
    md("## Section 8 — Feature Importance & SHAP"),
    code(FEAT_IMP),
    code(SHAP_CELL),
    md("## Section 9A — Full-Data Retrain"),
    code(FULL_RETRAIN),
    md("## Section 9B — Debug & Submission Validation"),
    code(DEBUG1),
    code(DEBUG2),
    code(DEBUG3),
    md("## Section 10 — Final Submission"),
    code(SUBMISSION),
    md("## Leaderboard Tracker & Model Comparison"),
    code(OPTUNA_PLOTS),
    code(LEADERBOARD),
    code(FALLBACK),
]
...
total_chars = sum(len(c["source"]) for c in code_cells)
print(f"Total source code: {total_chars:,} chars")
